...
if __name__ == "__main__":
    ...
    for var in var_col:
        ...
        del train_data, all_data
        gc.collect()

    head_col.extend(var_col)
    final_result = final_result[head_col]
    final_result.sort_values(["wtid", "ts"], inplace=True)
    final_result.to_csv("./result/horizontal_result_ver_rf.csv", encoding="utf8", index=False, float_format='%.2f')
    score_df.to_csv("./result/horizontal_score_ver_rf.csv", encoding="utf8", index=False, float_format='%.4f')
