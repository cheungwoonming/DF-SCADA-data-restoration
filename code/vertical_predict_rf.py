...
ext_enum_col = ["var010", "var013", "var019", "var023", "var032", "var039", "var041", "var048", "var049", "var050",
                "var054", "var058", "var059", "var060", "var063", "var064", "var065", "var068"]  # 数量多的取值极少，当做enum用分类模型
round_col = ["var008", "var025", "var026", "var062", "var067"]  # 结果保留一位或两位小数
threshold = 0.005
...
# 数值型的用均值
# TODO numeric_col = [i for i in var_col if i not in category_col and i not in ext_enum_col]
...
def _model_predict(all_feature, predict_feature, predict_col):
    ...
    if predict_col in category_col or predict_col in ext_enum_col:
        model = RandomForestClassifier(n_jobs=num_threads)
    else:
        model = RandomForestRegressor(n_jobs=num_threads)
    ...
    if predict_col in category_col or predict_col in ext_enum_col:
        # 用原始的全测试集
        if del_test_size > 0:
            test_pred = model.predict(test_x_org)
        test_y = test_y_org
        # 取回原来的值
    ...
    predict_target = model.predict(predict_x)
...
for var in var_col:
    ...
    for wtid in range(1, 34):
        ...
        predict_y, test_score = _model_predict(train_data, predict_data, var)
...
final_result.to_csv("./result/vertical_result_rf.csv", encoding="utf8", index=False, float_format='%.2f')
score_df.to_csv("./result/vertical_score_rf.csv", encoding="utf8", index=False, float_format='%.4f')
