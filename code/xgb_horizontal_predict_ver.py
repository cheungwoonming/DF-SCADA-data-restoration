...
ext_enum_col = ["var010", "var013", "var019", "var023", "var032", "var039", "var041", "var048", "var049", "var050",
                "var054", "var058", "var059", "var060", "var063", "var064", "var065"]  # 用作分类效果更好
large_limit_col = {"var019": 100, "var041": 10000, "var048": 100, "var059": 100, "var065": 100}  # 需要更大的number_limit
threshold = 0.005
argv = sys.argv
if len(argv) < 2:
...
if __name__ == "__main__":
    ...
    for var in var_col:
        ...
        for index, t in enumerate(tool.types):
            if var in t:
                break
        test_label_col = str(index) + "_test"
        temp_col = ["time", "wtid", test_label_col]
        temp_col.extend(var_col)
        all_data = data[temp_col]
        all_data = pd.merge(all_data, sub_data, "left", on=["time"])

        test_scores = []
        del sub_data, use_data
        gc.collect()
        ...
        for wtid in range(1, 34):
            ...
            train_data.drop(["time", "wtid", target_col], axis=1, inplace=True)
            predict_data.drop(["time", "wtid", target_col], axis=1, inplace=True)
    ...
    final_result.to_csv("./result/xgb_horizontal_result_ver.csv", encoding="utf8", index=False, float_format='%.2f')
    score_df.to_csv("./result/xgb_horizontal_score_ver.csv", encoding="utf8", index=False, float_format='%.4f')
