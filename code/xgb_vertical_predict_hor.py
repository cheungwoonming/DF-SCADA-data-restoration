...
if __name__ == "__main__":
    ...
    if method == 1:
        feature_dict = pickle.load(open(path + "/feature_dict_25_2_40.pkl", "rb"))
    else:
        feature_dict = pickle.load(open(path + "/feature_dict_25_3_60.pkl", "rb"))
    ...
    for wtid in range(1, 34):
        ...
        for var in var_col:
            ...
            use_col = feature_dict[str(wtid) + var].copy()  # TODO 修改使用哪些变量做特征
    ...
    final_result.to_csv("./result/xgb_vertical_result_hor{}.csv".format(method), encoding="utf8", index=False, float_format='%.2f')

    score_df.set_index("var", inplace=True)
    score_df = score_df.T
    score_df.reset_index(inplace=True)
    score_df.rename(columns={"index": "wtid"}, inplace=True)
    score_df.to_csv("./result/xgb_vertical_score_hor{}.csv".format(method), encoding="utf8", index=False, float_format='%.4f')
